...
## 이슈사항 및 리뷰
첫번째
초반 api 설계와 url 명세서를 작성했지만 시큐리티를 적용하면 403 오류 발생 
...
초기에는 기능을 많이 넣고 최대한 많이 구현하는 것이 프로젝트의 완성도를 높이는 것이라고 생각했습니다. 
그래서 세명임에도 불구하고 골프장과 쇼핑몰을 합친 프로젝트를 기획했습니다. 
길팀장님이 초기 설계의 중요성을 알려주었고, 함께 erd, 화면정의서, 요구사항 정의서, 테이블 명세서 등에 1주일이 조금 넘는 시간을 활용했습니다. 
저는 백엔드의 전반을 맡았기 때문에 erd, 테이블 명세서, api 명세서를 최대한 꼼꼼하게 작성했고 시큐리티를 위해 url 명세서까지 작성했습니다. 
하지만 실전에서는 403 오류로 겹치는 기능들은 거의 다 막혀 수정했지만 배포할 때에도 시큐리티로 기능들이 막혔습니다. 
저희 프로젝트는 백엔드와 프론트의 url이 똑같습니다. 예를 들어 프론트는 /member/join 이라면 백엔드에서는 api/member/join으로 
단순히 저희가 알아보기 쉽게만 적용한 것이 문제였습니다. 기능별로 백엔드에서 url을 나눠야 한다는 것을 
